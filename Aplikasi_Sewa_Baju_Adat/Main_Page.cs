...
namespace Aplikasi_Sewa_Baju_Adat
{
    public partial class Main_Page : Form
    {
        ...
        private void Total_Click_1(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Masukkan jumlah hari sewa", "Peringatan", MessageBoxButtons.OK);
                return;
            }
            if (string.IsNullOrWhiteSpace(Harga.Text))
            {
                MessageBox.Show("pilih baju adat", "Peringatan", MessageBoxButtons.OK);
                return;
            }
            if (string.IsNullOrWhiteSpace(Jumlah.Text))
            {
                MessageBox.Show("jumlah barang", "Peringatan", MessageBoxButtons.OK);
                return;
            }
            if (string.IsNullOrWhiteSpace(pengiriman.Text))
            {
                MessageBox.Show("pilih pengiriman", "Peringatan", MessageBoxButtons.OK);
                return;
            }
            ...
            if (string.IsNullOrWhiteSpace(metodepembayaran.Text))
            {
                MessageBox.Show("pilih metode pembayaran", "Peringatan", MessageBoxButtons.OK);
                return;
            }
        ...
        private void cetak_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Masukkan nama penyewa", "Peringatan", MessageBoxButtons.OK);
                return;
            }
            if (konfirmasibayar.Text == "Sudah Bayar")
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(connString))
                {
                    ...
                    {
                        conn.Open();
                        string query = "INSERT INTO sewa (nama_penyewa, baju_adat, ukuran, tanggal_sewa, jumlah_barang, jumlah_hari, pengiriman, alamat, total_harga, metode_bayar) " +
